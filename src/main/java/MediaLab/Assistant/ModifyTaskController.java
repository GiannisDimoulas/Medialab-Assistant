...
public class ModifyTaskController {
	...
	public void initialize() {
		...
		newTaskAnchor.sceneProperty().addListener((obs, oldScene, newScene) -> {
            if (newScene != null) {
            	...
            	buttons.minWidthProperty().bind(restOptions.widthProperty());
            	buttons.maxWidthProperty().bind(restOptions.widthProperty());
	...
	public void save() throws IOException {
		if (titleField.getText().trim().isEmpty()) {
			displayNewObjectPopUp("You cannot create a new task without a title.");
		} else if (TaskList.instance.get(titleField.getText().trim()) != null && !TaskList.instance.get(titleField.getText().trim()).equals(this.task)) {
			...
			String priority = pickPriority.getValue().substring(pickPriority.getValue().indexOf(": ") + 2);
			Status status;
			...
			if (status.equals(Status.COMPLETED) && notifications.size() != 0) {
				...
				this.task.changeCategory(CategoryList.instance.get(category));
				this.task.changePriority(PriorityList.instance.get(priority));
				this.task.setStatus(status);
				this.task.chooseNotifications(notifications);
				dashboardPressed();
			}
		}
	}
	...
	private void initializeFields() {
		...
		pickDeadline.setValue(this.task.getDeadline());
		if (this.task.getCategory() != null) {
			pickCategory.setValue(this.task.getCategory().getTitle());
		}
		pickPriority.setValue(PriorityList.instance.getLevel(this.task.getPriority()) + ": " + this.task.getPriority().getTitle());
		pickStatus.setValue(this.task.getStatus().getTitle());
		
		for (Notification notification: task.getNotifications()) {
			if (notification.getType().equals(NotificationType.ONE_DAY_BEFORE)) {
				OneDayBefore.setSelected(true);
			}
			if (notification.getType().equals(NotificationType.ONE_WEEK_BEFORE)) {
				OneWeekBefore.setSelected(true);
			}
			if (notification.getType().equals(NotificationType.ONE_MONTH_BEFORE)) {
				OneMonthBefore.setSelected(true);
			}
			if (notification.getType().equals(NotificationType.DUE_DAY)) {
				DueDay.setSelected(true);
			}
		}
	}
